...
dag = DAG(dag_id='detached_trade_to_s3_dag',
          default_args=args,
...
def task_detached_trade_data(input_year, input_month, **kwargs):
    export_file_path = './export/'
    export_file_name = 'detached_trade_'
...
t2 = PythonOperator(task_id='task_2',
                    provide_context=True,
                    python_callable=task_detached_trade_data,
                    op_kwargs={'input_year': Variable.get("arg_year"),
                               'input_month': Variable.get("arg_month")},
                    dag=dag)
