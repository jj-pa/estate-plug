...
dag = DAG(dag_id='officetel_contract_to_s3_dag',
          default_args=args,
...
def get_contract_frame(service_key, lawd_cd, deal_ymd):
    ...
    for row in items:
        ...
        item['보증금'] = int(item['보증금'].replace(',', ''))
        item['월세'] = int(item['월세'].replace(',', ''))
...
def task_officetel_contract_data(input_year, input_month, **kwargs):
    export_file_path = './export/'
    export_file_name = 'officetel_contract_'
...
t2 = PythonOperator(task_id='task_2',
                    provide_context=True,
                    python_callable=task_officetel_contract_data,
                    op_kwargs={'input_year': Variable.get("arg_year"),
                               'input_month': Variable.get("arg_month")},
                    dag=dag)
